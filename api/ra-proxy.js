...
const LISTINGS = `query($filters: FilterInputDtoInput, $page: Int, $pageSize: Int){
  eventListings(filters: $filters, pageSize: $pageSize, page: $page){
    data { event {
      ...
      images { filename type } venue { name } artists { name } genres { name }
    } }
    totalResults
  }
}`;
...
export default async function handler(req, res) {
  try {
    ...
    // One card per event, anchored to its START date. RA returns a row per day
    // for multi-day/after-midnight events, so dedupe by id and keep the earliest.
    const byId = new Map();
    for (const { event: e } of rows) {
      if (!e?.id) continue;
      const prev = byId.get(e.id);
      if (!prev || (e.date || "") < (prev.date || "")) byId.set(e.id, e);
    }

    const events = [...byId.values()]
      .map((e) => {
        // Prefer flyerFront; fall back to the images array (newer events).
        const candidates = [
          e.flyerFront,
          e.images?.find((i) => i.type === "FLYERFRONT")?.filename,
          e.images?.[0]?.filename,
        ].filter(Boolean);
        const flyer = candidates.find((u) => /^https?:\/\//.test(u)) || "";
        return {
          id: String(e.id),
          title: e.title,
          ...
          // Stream posters through our own function so RA's CDN can't refuse the hotlink.
          image: flyer ? `/api/ra-image?u=${encodeURIComponent(flyer)}` : "",
